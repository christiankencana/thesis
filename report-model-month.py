...
if uploaded_file:
    ...
    actual_data = extended_data[extended_data[selected_column] != 0]
    ...
    mean_MAD_rounded = round(mean_MAD, 5)
    mean_MSE_rounded = round(mean_MSE, 5)
    mean_MAPE_rounded = round(mean_MAPE, 5)
    ...
    st.write(f"Plot: Error and Evaluation Metrics (Error, MAD, MSE, MAPE) for {selected_column}")
    ...
    ax.set_title(f'Error and Evaluation Metrics (Error, MAD, MSE, MAPE) for {selected_column}')
    ax.legend()
    plt.xticks(rotation=45)
    st.pyplot(fig)

    # Plot: Error Evaluation Metrics (MAD)
    st.write(f"Plot: Error and Evaluation Metrics (MAD) for {selected_column}")
    fig, ax = plt.subplots(figsize=(14, 8))
    # ax.plot(extended_data['bulan_tahun'].astype(str), extended_data['Error'], label='Error', marker='o', color='red')
    ax.plot(extended_data['bulan_tahun'].astype(str), extended_data['MAD'], label='MAD', marker='x', color='blue')
    ax.set_xlabel('Month-Year')
    ax.set_ylabel('Error Metrics')
    ax.set_title(f'Error and Evaluation Metrics (MAD) for {selected_column}')
    ax.legend()
    plt.xticks(rotation=45)
    st.pyplot(fig)
    
    # Plot: Error Evaluation Metrics (MSE)
    st.write(f"Plot: Error and Evaluation Metrics (MSE) for {selected_column}")
    fig, ax = plt.subplots(figsize=(14, 8))
    ax.plot(extended_data['bulan_tahun'].astype(str), extended_data['MSE'], label='MSE', marker='s', color='green')
    ax.set_xlabel('Month-Year')
    ax.set_ylabel('Error Metrics')
    ax.set_title(f'Error and Evaluation Metrics (MSE) for {selected_column}')
    ax.legend()
    plt.xticks(rotation=45)
    st.pyplot(fig)
    
    # Plot: Error Evaluation Metrics (MAPE)
    st.write(f"Plot: Error and Evaluation Metrics (MAPE) for {selected_column}")
    fig, ax = plt.subplots(figsize=(14, 8))
    ax.plot(extended_data['bulan_tahun'].astype(str), extended_data['Error'], label='Error', marker='o', color='red')
    ax.plot(extended_data['bulan_tahun'].astype(str), extended_data['MAD'], label='MAD', marker='x', color='blue')
    ax.plot(extended_data['bulan_tahun'].astype(str), extended_data['MSE'], label='MSE', marker='s', color='green')
    ...
    ax.set_xlabel('Month-Year')
    ax.set_ylabel('Error Metrics')
    ax.set_title(f'Error and Evaluation Metrics (MAPE) for {selected_column}')
    ax.legend()
    plt.xticks(rotation=45)
    st.pyplot(fig)